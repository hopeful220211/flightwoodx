...
>
> 更新时间：2026-09-15；本地流程升级与云端启用状态见 CURRENT_STATUS.md
>
...
每次上传、更新或上线必须先遵循[固定发布规则](release-procedure.md)。统一入口为 `pnpm release` 或 `pnpm release:check`（二者等价，只读核对证据，不运行完整本地 CI），以及 `pnpm release --publish`（已授权的前端发布）。工作在独立 `codex/` 分支，保留当前生产分支的历史。

程序执行的顺序：

1. 完成相关开发测试，先核对生产祖先与单父历史，再提交最终版本。需要云端完整检查时直接推送开发分支，不先重复完整本地 CI。
...
新 CI 仅在 `Production build` 构建一次，生成带完整 SHA、文件清单和摘要的归档。三个普通浏览器分组和一个隐私分组分别在独立运行器/API/Mongo 中检查同一归档；每组先严格解包验证。`Browser core flow` 只有在所有分组成功后才能保留原归档为 `web-release-<SHA>`，不再重打包。旧串行 CI 证据仍可读取；新证据必须包含四个成功分组及各自真实执行步骤，缺组、错版本或跳过步骤均拒绝。
...
2. 完成工作流及证据校验器的相关测试，提交并普通推送开发分支，等待该确切 SHA 的八项真实 GitHub 检查和归档上传通过；本地完整交付已检查的内容不再另跑一轮本地完整 CI，没有云端成功结果不继续。
