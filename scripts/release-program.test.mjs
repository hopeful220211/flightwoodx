...
async function fixture(t, overrides = {}) {
  ...
  const run = async (command, args) => {
    ...
    if (command === 'git') {
      ...
      if (key.startsWith('rev-list --merges ')) return { code: 0, stdout: overrides.merges ? `${NEW}\n` : '' };
...
for (const [name, overrides, code] of [
  ...
  ['merge commits before candidate CI', { merges: true, noEvidence: true }, 'nonlinear_release_history'],
