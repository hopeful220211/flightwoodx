...
for (const viewport of [
  ...
  test(`concise product copy and actionable design feedback at ${viewport.width}×${viewport.height}`, async ({ page, context }) => {
    ...
    // GuidedDesignPage automatically saves each edit.
    expect((await savingOfficial).ok()).toBe(true)
    await page.goto(`/design/export-preview/${officialId}`)
    await report.scrollIntoViewIfNeeded()
    await expect(report.getByText('当前没有待修改项目', { exact: true })).toBeVisible()
    ...
    await otherResults.locator('summary').click()
    await expect(otherResults).toHaveJSProperty('open', false)
    const structureSummary = page.locator('section[aria-labelledby="structure-summary-heading"]')
    await structureSummary.scrollIntoViewIfNeeded()
    await expect(structureSummary.getByText('零件总数', { exact: true })).toBeVisible()
    await expect(structureSummary.getByText('目录质量小计', { exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: '零件清单', exact: true })).toBeVisible()
