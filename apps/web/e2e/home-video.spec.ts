...
for (const viewport of [
  ...
  test(`homepage video plays inline without changing its frame at ${viewport.width}×${viewport.height}`, async ({ page }) => {
    ...
    expect(poster!.width, 'Use the large DJI-style product frame within 1200 px').toBeLessThanOrEqual(1200)
    expect(poster!.width).toBeGreaterThan(viewport.width * 0.75)
    expect(poster!.x).toBeGreaterThanOrEqual(0)
    expect(poster!.x + poster!.width).toBeLessThanOrEqual(viewport.width)
    const boundary = hero!.y + hero!.height
    expect(poster!.y - boundary, 'Separate the two product panels with a narrow gutter').toBeGreaterThanOrEqual(12)
    expect(poster!.y - boundary).toBeLessThanOrEqual(32)
