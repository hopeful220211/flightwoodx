...
export function CustomPartInspector({ instance }: { instance: DesignPartInstance }) {
  ...
  return <div className="min-w-0 flex-1 text-xs">
    <button type="button" className="w-full truncate text-left text-sm font-semibold" onClick={() => useDesignStore.getState().setSelectedInstanceId(instance.instanceId)}>{usable ? query.data!.name : '自制零件'}</button>
    ...
    <div className="mt-2 grid grid-cols-3 gap-1">
      {(['X', 'Y', 'Z'] as const).map((axis, index) => <label key={axis} className="min-w-0">{axis} (mm)<input aria-label={`自制零件 ${axis} 位置（毫米）`} disabled={!!instance.attachedTo} className="site-form-control min-h-11 w-full rounded border px-1 py-1 disabled:bg-slate-100" type="number" min={-1000} max={1000} step={1} value={+(instance.position[index] * 1000).toFixed(2)} onChange={event => {
