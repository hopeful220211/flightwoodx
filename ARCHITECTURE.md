...
- `@fwx/geometry.validateJointGuides` 被前端和 API 共用：板内槽必须对应完整矩形内孔，边缘槽必须对应指定入口的连续三边 U 形切口；损坏、重叠、尺寸或方向不符会阻止保存。旧记录可缺省标注；PUT 省略时保留原标注并以新几何重新复核，显式 `[]` 清空，读取复核后发生并发变化则返回 409。标注不是配对关系，仍不允许自制件自动连接或声称制造检查已通过。
