...
> 状态：当前架构说明
>
> 更新时间：2026-09-14（补充自制插槽设计标注与共享依赖；具体功能进度见 CURRENT_STATUS.md）
>
> 本轮结构基线：`6d0f17c`
>
...
应用可以依赖共享包；共享包不能依赖应用。当前共享包内部有 `@fwx/shared → @fwx/parts-schema` 和 `@fwx/geometry → @fwx/parts-schema` 两条工作区依赖。后者复用插槽标注契约与唯一板厚常量，不形成循环依赖；geometry 的 CJS 构建将该 TypeScript 契约打包，供 API 直接加载。跨应用的数据形状必须来自共享包并在运行时校验。
...
- 官方零件元数据来自 registry；用户零件来自正式 `CustomPart` 记录。自制零件由 `features/partStudio/customAssembly.ts` 校验来源、版本、归属与几何，可进入按步骤拼装和自由拼装；作品只存 `source` 引用，不复制源几何。主机身、起落架、保护板、连接/装饰件均支持，不支持自制电子件。
...
- `UserPartDef.jointGuides` 是可选的槽设计标注，保存稳定 id、边缘/板内槽类型、槽矩形左上角毫米坐标、长度、长轴及入口方向；窄边统一由 2mm 板厚常量派生。普通切孔不产生标注，也不借用官方 `sockets` 类型。保存时标注和轮廓减去同一局部原点，按 0.01mm 序列化；我的零件从服务端 DTO 读取已保存的位置与方向。
- 独立插接口工具由 `canvas/insertionSlot.ts` 在已编译外轮廓上定位边缘，不吸附参考框或内孔。拖动创建水平/竖直、2mm 宽的切除，并在提交前重编译复核。曲线/斜边入口允许两条槽边长度不同，`lengthMm` 取较远入口端至固定槽底的轴向距离；槽底中点由位置、长轴和入口方向派生，不新增重复存储字段。共享校验仍拒绝槽底破损、侧壁被横向切穿与反向标注。
- `@fwx/geometry.validateJointGuides` 被前端和 API 共用：板内槽必须对应完整矩形内孔，边缘槽必须对应指定入口的连续三边 U 形切口；损坏、重叠、尺寸或方向不符会阻止保存。旧记录可缺省标注；PUT 省略时保留原标注并以新几何重新复核，显式 `[]` 清空，读取复核后发生并发变化则返回 409。两种拼装模式中，用户选择双方真实接口后由共享连接计算对齐槽底并设定垂直；标注本身不表示已连接。API复核源所有权、版本、几何、接口占用和连接姿态，连接不代表制造检查通过。
- 预览、自由拼装与封面复用 `buildCustomGeometry`，按毫米→米生成真实2mm板厚；相机适配显示范围而不缩放几何。官方与自制件共用木纹材质工厂。服务端和显示端复用共享严格SVG解析与范围/复杂度检查；失败不删除原记录或替换来源。
- 混合拼接不得假定官方 GLB 接口坐标轴等于木板坐标轴。受控生成器从实际网格和节点层级提取板面法线；插入轴异常时从槽底侧面恢复方向。共享求解器统一板面方向后对齐槽底；客户端与 API 使用同一目录。`boardNormal` / `mouthDirection` 仅为受控运行时元数据，不增加用户输入或数据库字段。点击和鼠标/触控拖入复用账号源、占用规则与共享计算。历史混合连接仅在姿态精确符合旧算法时纠正，连带子件保持刚性关系；不猜测独立零件的连接。
...
- `@fwx/geometry` 处理已有二维轮廓，并复核槽设计标注是否与真实孔槽对应；不从三维外观推断制造轮廓或连接兼容性。
